...
export class Camp {
  constructor(scene, world, player, hooks) {
    ...
    this.levels = { home: 0, chest: 0, furnace: 0, boat: 0, tower: 0 };
  ...
  // max-hp bonus from the home building (cabin +15, stone house +40)
  homeHpBonus() { return this.levels.home >= 3 ? 40 : this.levels.home >= 2 ? 15 : 0; }
  ...
  _placeMesh(id) {
    ...
    if (id === 'home') {
      ...
      if (lvl === 3) { // stone house: recolor the cottage walls
        mesh.traverse(o => { if (o.isMesh && o.material?.color?.getHex?.() === 0x6e4d2a) {
          o.material = o.material.clone(); o.material.color.setHex(0x8f8a7c);
        } });
      }
      mesh.rotation.y = 0.4;
    ...
    this.scene.add(mesh);
    this.meshes[id] = mesh;
    this.world.obstacles.push({ x: spot.x, z: spot.z, r: id === 'home' ? 2.2 : 1.1 });
  }
